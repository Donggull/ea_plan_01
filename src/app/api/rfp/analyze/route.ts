...
function simulateAIAnalysis(content: string): RequirementExtraction[] {
  ...
  const keywords = {
    functional: ['로그인', '회원가입', '결제', '주문', '상품', '장바구니', '검색', '게시판', '댓글', '리뷰'],
    non_functional: ['성능', '보안', '확장성', '접근성', '반응형', '호환성', '가용성', '유지보수'],
    business: ['매출', '고객', '브랜드', '마케팅', '분석', '리포트', '대시보드', 'KPI'],
    technical: ['API', '데이터베이스', '클라우드', '배포', '모니터링', '로그', 'SSL', '백업'],
    ...
    keywordList.forEach(keyword => {
      if (content.toLowerCase().includes(keyword)) {
        const requirement: RequirementExtraction = {
          id: (id++).toString(),
          title: generateRequirementTitle(keyword, type as any),
          description: generateRequirementDescription(keyword, type as any),
          requirement_type: type as any,
...
function generateRequirementTitle(keyword: string, type: string): string {
  ...
  return templates[type as keyof typeof templates][keyword] || `${keyword} 관련 요구사항`
}

function generateRequirementDescription(keyword: string, type: string): string {
  const descriptions = {
    functional: {
      '로그인': '사용자가 이메일/비밀번호 또는 소셜 계정을 통해 시스템에 로그인할 수 있는 기능',
      '회원가입': '신규 사용자가 계정을 생성하고 이메일 인증을 통해 회원가입할 수 있는 기능',
  ...
  return descriptions[type as keyof typeof descriptions]?.[keyword] || 
    `${keyword}에 대한 상세한 요구사항을 정의하고 구현해야 합니다.`
}

function generatePriority(keyword: string): 'critical' | 'high' | 'medium' | 'low' {
...
function generateAcceptanceCriteria(keyword: string): string[] {
  const criteria = {
    '로그인': ['이메일/비밀번호 검증', '로그인 실패 시 에러 메시지', '로그인 성공 시 토큰 발급'],
    '결제': ['PG사 연동', '결제 성공/실패 처리', '결제 내역 저장'],
    '상품': ['상품 CRUD 기능', '이미지 업로드', '재고 관리'],
    '성능': ['페이지 로딩 3초 이내', '동시 접속자 1000명 처리', '서버 응답시간 1초 이내']
  }
  
  return criteria[keyword as keyof typeof criteria] || [`${keyword} 기능 동작`, `${keyword} 에러 처리`, `${keyword} 테스트 완료`]
}

function generateRiskLevel(keyword: string): 'critical' | 'high' | 'medium' | 'low' {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        by_type: {
          functional: requirements.filter(r => r.requirement_type === 'functional').length,
          non_functional: requirements.filter(r => r.requirement_type === 'non_functional').length,
          business: requirements.filter(r => r.requirement_type === 'business').length,
          technical: requirements.filter(r => r.requirement_type === 'technical').length,
          constraint: requirements.filter(r => r.requirement_type === 'constraint').length
        },
        by_priority: {
        ...
        estimated_total_effort: requirements.reduce((sum, r) => sum + r.estimated_effort, 0),
        average_business_value: Math.round(requirements.reduce((sum, r) => sum + r.business_value, 0) / requirements.length * 10) / 10
      }
    })
