...
function generateCompetitors(category: string) {
  ...
  return competitorSets[category as keyof typeof competitorSets] || competitorSets.development
}
    ...
    proposal: [
      {
        ...
        description: 'ChatGPT, Claude 등 AI 도구를 활용한 제안서 작성이 급속히 증가하고 있음'
      },
      {
        trend: '협업 도구 통합',
    ...
    development: [
      {
        ...
        impact: 'high' as const,
        description: 'Next.js, Nuxt.js 등 메타프레임워크 사용률이 70% 이상 증가'
      },
      {
    ...
    operation: [
      {
        trend: 'DevOps 자동화',
        impact: 'high' as const,
        description: 'CI/CD 파이프라인 구축이 필수가 되어 운영 효율성 크게 향상'
      },
      {
        trend: '모니터링 및 옵저빌리티',
        impact: 'medium' as const,
        description: 'Prometheus, Grafana 등을 활용한 실시간 모니터링 중요성 증대'
      }
    ]
  }
...
function generateTechnologyStack(category: string) {
  ...
  return techStacks[category as keyof typeof techStacks] || techStacks.development
}
...
function generatePricingAnalysis(category: string) {
  const pricingData = {
    proposal: {
      ...
      recommendedPricing: {
        strategy: '가치 기반 가격책정',
        justification: '경쟁사 대비 15-20% 할인된 가격으로 시장 진입 후, 품질 입증을 통해 점진적 인상'
      }
    },
    development: {
      ...
      recommendedPricing: {
        strategy: '단계별 가격책정',
        justification: 'MVP 단계는 경쟁력 있는 가격, 고도화 단계에서는 프리미엄 가격 적용'
      }
    },
    operation: {
      ...
      recommendedPricing: {
        strategy: '구독 기반 티어링',
        justification: '기본/표준/프리미엄 3단계 구성으로 고객 선택권 제공 및 업셀링 기회 창출'
      }
    }
  }

  return pricingData[category as keyof typeof pricingData] || pricingData.development
}

function generateTimeline(category: string) {
  const timelines = {
    proposal: [
      { phase: '요구사항 분석', duration: '1-2주', description: 'RFP 분석 및 고객 니즈 파악' },
      { phase: '시장 조사', duration: '1주', description: '경쟁사 분석 및 기술 트렌드 조사' },
      { phase: '제안서 작성', duration: '2-3주', description: '초안 작성, 검토, 수정 과정' },
      { phase: '최종 검토', duration: '3-5일', description: '품질 확인 및 최종 수정' }
    ],
    development: [
      { phase: '기획 및 설계', duration: '2-4주', description: '요구사항 정의, 시스템 아키텍처 설계' },
      { phase: '개발', duration: '8-16주', description: '프론트엔드 및 백엔드 개발, API 구현' },
      { phase: '테스팅', duration: '2-4주', description: '단위/통합/사용자 테스트' },
      { phase: '배포 및 운영', duration: '1-2주', description: '프로덕션 배포 및 모니터링 설정' }
    ],
    operation: [
      { phase: '인프라 구축', duration: '1-2주', description: '서버 환경 구성 및 모니터링 설정' },
      { phase: '서비스 이관', duration: '1주', description: '기존 서비스에서 새 환경으로 이관' },
      { phase: '안정화', duration: '2-4주', description: '성능 튜닝 및 안정성 확보' },
      { phase: '지속적 운영', duration: '지속적', description: '24/7 모니터링 및 유지보수' }
    ]
  }
...
function generateRecommendations(category: string, keywords: string[]) {
  ...
  let recommendations = baseRecommendations[category as keyof typeof baseRecommendations] || baseRecommendations.development
